...
CEBPUB 详情默认由 FusionBid 自动启动并复用独立的专用 Chrome（失败时改用 Edge），无需安装扩展或逐条点击。系统自动打开 UUID 对应公告、读取内嵌 PDF 的全部页、校验标题与页码，再进入 AI 抽取和证据校验；专用配置不读取日常浏览器 Cookie，也不进入 Git。仅当官方明确要求验证码时才显示专用浏览器供用户完成验证，系统随后自动继续，不绕过访问控制。`browser_extension/ctbpsp_capture` 与官方文件导入仅保留为开发诊断兜底。

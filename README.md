...
CEBPUB 详情默认由 FusionBid 自动启动并复用独立的专用 Chrome（失败时改用 Edge），无需安装扩展或逐条点击。系统优先从 PDF.js 内存读取 PDF 字节，再使用 PyMuPDF/pypdf 与本地 OCR；明确的无效/损坏 PDF 会快速终止并进入 24 小时自动冷却，临时未就绪才重建标签页重试一次。系统校验 UUID、标题和页码后再进入 AI 抽取；专用配置不读取日常浏览器 Cookie，也不进入 Git。仅当官方明确要求验证码时才显示专用浏览器供用户完成验证，不绕过访问控制。`browser_extension/ctbpsp_capture` 与官方文件导入仅保留为开发诊断兜底。

抽取采用 **规则快速提取 → 关键字段缺口交给 AI → 原文证据校验 → 规则结果保底**。系统把“公告生命周期”和“采购方式”独立保存：机会公告才生成参与建议；结果、更正、终止/废标公告作为项目生命周期情报展示，中标金额不会写入预算字段。相同正文、抽取版本、模型和企业画像指纹均未变化时复用证据抽取，不重复调用模型；连续两次模型超时后，本轮后续公告使用规则结果并标记待补充。

普通检索支持快速（每源 8 条）、标准（每源 30 条，默认）和深度（每源 500 条）。只要有详情因上限跳过，就返回 `truncated=true` 并展示发现、实际采集和跳过数量。任务列表的“重新采集未去重完整报告”是独立完整快照：重新抓取、保留每个来源记录、不读写 DeliveryHistory；达到每源 500 条上限会明确标记截断。
...
当前仓库最近一次结果：后端 **199 passed**，前端 lint 通过；完整 build、Docker Compose 与 Word/浏览器验收结果见 [测试报告](docs/测试报告.md)。
